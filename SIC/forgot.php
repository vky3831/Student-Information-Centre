...
if(isset($_POST["submit"])){
	...
	$query = "SELECT * FROM student WHERE reg='$reg' AND sec_ques='$sec_ques' AND sec_ans='$sec_ans' ";
	$exe = mysqli_query($connection ,$query);
	if(mysqli_fetch_assoc($exe)){
		$query = "UPDATE student SET password='$password' WHERE reg='$reg' ";
		$exe = mysqli_query($connection ,$query);
		if($exe){
		echo "<script>alert('Password Changed')</script>";
		echo "<script>window.open('login_as_student.php', '_self')</script>";
	}
		else{
...
<ul>
  ...
  <li><a href="forgot.php" style="background-color: #006CFF">Password Recovery</a></li>
</ul>
</div>
<div class="main">
<div class="click_forgot">
	<div class="box">
  <form class="form-inline" action="forgot.php" method="POST">

		<label for="reg">Registration no.:</label><br>
    <input type="text" id="reg" name="reg" placeholder="Enter Registration no." onfocusout="validate_reg()" required><br>
    ...
    <select name="sec_ques" id="sec_ques">
      <option value="fav_book">Which is favourite author?</option>
